/* ===========================================================
   أقسام وألعاب مخيم سردا الصيفي
   -----------------------------------------------------------
   SECTIONS: الأقسام التي يختار منها الطالب (مادة/مجال).
   الألعاب الدائمة محفوظة في games-data.json (يديره المساعد للجميع).
   FALLBACK_GAMES: قائمة احتياطية تظهر فقط إذا تعذّر تحميل الملف.
   =========================================================== */

/* الأقسام المتاحة
   desc   = سطر المعلومات الذي يظهر على بطاقة القسم
   always = يظهر دائماً حتى لو فارغاً (غيره يختفي إذا لا ألعاب فيه) */
window.SECTIONS = [
  { id: "english", title: "اللغة الإنجليزية",   icon: "🔤", color: "green",  desc: "حروف وكلمات وألعاب بالإنجليزية", always: true },
  { id: "arabic",  title: "اللغة العربية",      icon: "📖", color: "teal",   desc: "قراءة وإملاء ونحو" },
  { id: "math",    title: "الرياضيات",          icon: "🧮", color: "blue",   desc: "أرقام وحساب وتحدّيات ذكاء" },
  { id: "science", title: "العلوم",             icon: "🔬", color: "purple", desc: "تجارب واكتشافات ممتعة" },
  { id: "social", title: "الدراسات الاجتماعية", icon: "🧭", color: "gold",   desc: "وطن وتاريخ وجغرافيا" },
  { id: "islamic", title: "التربية الإسلامية",  icon: "🕌", color: "teal",   desc: "قرآن وأخلاق وسيرة" },
  { id: "tech",   title: "التكنولوجيا والبرمجة", icon: "💻", color: "blue",   desc: "حاسوب وبرمجة وتفكير منطقي", always: true }
];

/* ===== مصدر الألعاب =====
   ضعي هنا رابط الـ Web App الخاص بشيت Google (المنتهي بـ /exec)
   بمجرد وضعه: تصبح القراءة والحفظ من الشيت مباشرةً.
   إذا تُرك فارغاً: يُستخدم الملف الثابت games-data.json كبديل. */
window.GAMES_API_URL = "https://script.google.com/macros/s/AKfycbzvEZaU39m64iiIVLceiMOvy36kx1u1fmkPVAPi2OyqgotAo-GTeidk__aoCbHUenOo/exec";

/* بديل ثابت يُستخدم فقط إذا لم يُضبط GAMES_API_URL */
window.GAMES_DATA_URL = "games-data.json";

/* قائمة احتياطية تظهر فقط إذا تعذّر تحميل games-data.json */
window.FALLBACK_GAMES = [];
